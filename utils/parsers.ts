import { ParkingLayout, ElementType, ElementTypes, LayoutElement } from '../types';
...
export function parseCustomLayout(json: any): ParkingLayout {
  ...
  input.parking?.forEach(p => convertBBox(p.id, ElementTypes.PARKING_SPACE, p.bbox, p.id));
  input.walls?.forEach(w => convertBBox(w.id, ElementTypes.WALL, w.bbox, w.id));
  input.entrances?.forEach(e => {
    const type = e.type === 'out' ? ElementTypes.EXIT : ElementTypes.ENTRANCE;
